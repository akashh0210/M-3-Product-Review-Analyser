...
Merge similar themes together. Keep the total to ${maxThemes} or fewer.
Aggregate the counts and combine the reviewIndices.
Ensure sentiments and summaries are reflective of the merged content.
...
${JSON.stringify(allThemes, null, 2)}`;
  ...
  try {
    const data = JSON.parse(response);
    const consolidated = data.themes || [];
    return consolidated.sort((a: any, b: any) => b.count - a.count).slice(0, maxThemes);
  } catch (err) {
    console.error('❌ Failed to parse LLM response for theme consolidation');
    // Fallback: just take top themes from the flattened list (not ideal but safe)
